...
export default function HomePage() {
  ...
  const handleStartQuiz = () => {
    if (selectedSubject && selectedYear && selectedCourse) {
      router.push(
        `/quiz?subject=${selectedSubject}&year=${selectedYear}&course=${selectedCourse}`,
      );
    }
  };
      ...
      <header className='sticky top-0 z-10 glass'>
        <div className='container mx-auto px-4 py-4 flex items-center justify-between'>
          ...
          <nav className='hidden md:flex space-x-4'>
            <Button
              variant='ghost'
              className='text-primary hover:text-primary-foreground hover:bg-primary rounded-full transition-colors duration-200'
            >
              Dashboard
            </Button>
            <Button
        ...
        {mobileMenuOpen && (
          <nav className='md:hidden flex flex-col space-y-2 p-4 bg-card border-t border-primary'>
            <Button
              variant='ghost'
              className='text-primary hover:text-primary-foreground justify-start rounded-full'
            >
              Dashboard
            </Button>
            <Button
              ...
              >
                ...
                <CardContent>
                  <p className='text-3xl font-bold text-primary mb-2'>
                    ...
                    {quiz.score > 70 ? (
                      <ChevronUp className='inline-block ml-2 text-green-500' />
                    ) : (
                      <ChevronDown className='inline-block ml-2 text-red-500' />
                    )}
                  </p>
        ...
        <section className='text-center mb-20'>
          ...
          <div className='grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-8 max-w-4xl mx-auto'>
            {[
              ...
              <Card
                key={index}
                className='bg-card border-primary hover:shadow-xl transition-shadow duration-300 rounded-3xl overflow-hidden group'
              >
                <CardContent className='p-6 flex flex-col items-center text-center'>
                  <div
                    className={`${action.color} p-4 rounded-full mb-4 group-hover:scale-110 transition-transform duration-300`}
                  >
                    <action.icon className='h-8 w-8 text-primary' />
                  </div>
                  <h3 className='text-lg font-semibold text-primary group-hover:text-primary-600 transition-colors duration-300'>
                    {action.title}
                  </h3>
                </CardContent>
              </Card>
